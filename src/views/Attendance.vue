...
export default {
    setup() {
        ...
        function deleteGuest(id) {
            axios({
                url: this.baseUrl + '/guest/delete-guest',
                method: 'POST',
                data: { id: id },
                responseType: 'json'
            }).then((response) => {
    ...
    components: { InsertModalComponent, AlertComponent }
}
</script>
<template>
    <main class="h-full">
            ...
            <table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400 p-2">
                ...
                <tbody>
                    ...
                    >
                        ...
                        <td class="px-4 py-4">
                            <button @click="deleteGuest(guest._id)" class="font-medium text-blue-600 dark:text-blue-500 hover:underline">
        ...
        <InsertModalComponent :showModal="showModal" v-on:close-modal="closeModal" v-on:refresh-table="onRefreshTable"/>

        <AlertComponent :message="message" v-on:close-alert="closeAlert()"  />
